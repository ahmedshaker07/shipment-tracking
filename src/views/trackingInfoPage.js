...
function TrackingInfoPage({intl,...props}) {
    ...
    const fetchShipmentData = async ()=>{
        ...
        .then(res=>{
            ...
            const { CurrentStatus: { state } } = res.data;
            if(state==="DELIVERED") {
                setActiveStep(4);
                setStatus("finished");
            }
    ...
    const getFormattedTransitEvents= ()=>{
        let newShipmentData = [];
    ...
    return(
        ...
        <div className="delivery-main">
           <div className="delivery-status">
                <div className="info">
                    <Info header={intl.formatMessage({id: 'info.trackingNumber'},{trackingNo: shipmentDetails?.TrackingNumber})} status={status} 
                            description={intl.formatMessage({id: `${shipmentDetails?.CurrentStatus?.state}`})} />
                    <Info header={intl.formatMessage({id: 'info.latestUpdate'})} date={formatLatestUpdateDate(shipmentDetails?.CurrentStatus?.timestamp)}/>
                    <Info header={intl.formatMessage({id: 'info.merchant'})} description={intl.formatMessage({id: 'info.merchantName'})}/>
                    <Info header={intl.formatMessage({id: 'info.deliveryDate'})} date={formatPromisedDate(props)}/>
                </div>
                <div className="progress" dir="ltr">
                    <Steps current={activeStep} status={status} size="small" labelPlacement="vertical" className={getStepperClassName()}>
           ...
           <div className="delivery-info">
               <div className="delivery-progress">
                   ...
                   <Table columns={columns} dataSource={getFormattedTransitEvents()} pagination={false} scroll={{ x: true }}/>
               </div>
